...
<body>
    ...
    <main>
        <div class="container">
            <form action="index.php?action=editProduct&id=<?= $product['id'] ?>" method="POST" class="product-form">
                <label for="nom">Nom du produit:</label><br>
                <input type="text" id="nom" name="nom" value="<?= htmlspecialchars($product['nom']) ?>" required><br><br>

                <label for="description">Description:</label><br>
                <textarea id="description" name="description" required><?= htmlspecialchars($product['description']) ?></textarea><br><br>

                <label for="prix">Prix:</label><br>
                <input type="number" id="prix" name="prix" value="<?= htmlspecialchars($product['prix']) ?>" step="0.01" min="0" required><br><br>

                <label for="quantite">Quantité:</label><br>
                <input type="number" id="quantite" name="quantite" value="<?= htmlspecialchars($product['quantite']) ?>" min="1" required><br><br>
